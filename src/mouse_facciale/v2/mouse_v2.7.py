...
class HeadMouseController:
    def __init__(self, show_window=True):
        ...
        print("Click SINISTRO con occhio sinistro | Click DESTRO con occhio destro")
        print("Punto di riferimento: punta del naso")

    def calculate_eye_aspect_ratio(self, landmarks, eye='left'):
        """Calcola EAR per un occhio specifico."""
        try:
            if eye == 'right':
    ...
    def detect_blink_left(self, landmarks):
        ...
        stable_ear = np.mean(list(self.left_ear_history)) if len(self.left_ear_history) >= 2 else ear
    ...
    def detect_blink_right(self, landmarks):
        ...
        # Stabilizza con media mobile
        stable_ear = np.mean(list(self.right_ear_history)) if len(self.right_ear_history) >= 2 else ear
    ...
    def perform_click(self, click_type='left'):
        ...
        try:
            ...
            print(f"Errore click {click_type}: {e}")
            return False
    ...
    def draw_interface(self, frame, tracking_point, landmarks=None):
        ...
        if not self.center_calculated:
            ...
            if landmarks is not None and not self.paused:
                ...
                left_eye_color = (255, 0, 0) if self.left_eye_closed else (0, 255, 0)  # Rosso se chiuso, verde se aperto
                ...
                right_eye_color = (0, 0, 255) if self.right_eye_closed else (0, 255, 0)  # Blu se chiuso, verde se aperto
            ...
            # Legenda click
            cv2.putText(frame, "Occhio SX = Click SX", (20, 110), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
            cv2.putText(frame, "Occhio DX = Click DX", (20, 130), 
...
def main():
    ...
    print("\n👁️ CLICK:")
    print("Occhio SINISTRO = Click SINISTRO del mouse")
    print("Occhio DESTRO = Click DESTRO del mouse")
        ...
        while True:
            ...
            if results.multi_face_landmarks:
                ...
                if not controller.center_calculated:
                    ...
                    controller.detect_blink_left(landmarks_np)
                    controller.detect_blink_right(landmarks_np)
