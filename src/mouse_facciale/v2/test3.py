...
class SimpleNoseMouseController:
    def __init__(self, show_window=True):
        ...
        :param show_window: Se True, mostra la finestra della webcam. Se False, funziona in background.
        """
        ...
        self.last_nose_pos = np.array([320.0, 240.0]) # Posizione iniziale di default per il disegno se non c'è il naso
        ...
        self.show_window = show_window # Flag per mostrare/nascondere la finestra
    ...
    def detect_left_eye_blink(self, landmarks):
        ...
        if (avg_ear < self.eye_closed_threshold and 
            ...
            try:
                # Esegui il click alla posizione corrente del cursore
                pyautogui.click(x=int(self.current_cursor_pos[0]), y=int(self.current_cursor_pos[1]))
                self.last_click_time = current_time
                print(f"\nClick eseguito a {int(self.current_cursor_pos[0])},{int(self.current_cursor_pos[1])}! EAR: {avg_ear:.4f}")
                return True
    ...
    def update_cursor_position(self, nose_pos):
        ...
        if distance < self.deadzone_radius:
            # Se il naso è nella deadzone, il cursore non si muove.
            # Questo evita piccoli movimenti involontari.
            return

        # Calcola movimento
        movement_factor = (distance - self.deadzone_radius) / distance # Proporzionale alla distanza dalla deadzone
    ...
    def draw_interface(self, frame, nose_pos, landmarks=None, last_click_detected=False):
        """
        Disegna l'interfaccia visuale sul frame, solo se show_window è True.
        """
...
def main():
    ...
    # Chiedi all'utente se vuole mostrare la finestra
    while True:
        user_input = input("Vuoi mostrare la finestra della webcam? (s/n): ").lower().strip()
        if user_input == 's':
            show_window = True
            break
        elif user_input == 'n':
    ...
    controller = SimpleNoseMouseController(show_window=show_window)
    ...
    if show_window:
        ...
        print("Il programma sta girando in background. Premi CTRL+C nella console per uscire.")
    ...
    try:
        ...
        while True:
            ...
            if results.multi_face_landmarks:
                ...
                controller.last_nose_pos = nose_pos.copy() # Aggiorna sempre la posizione del naso
                ...
                # Disegna l'interfaccia solo se la finestra è visibile
                if show_window:
                    controller.draw_interface(frame, controller.last_nose_pos, landmarks_np, last_click_detected)
            else:
                # Se non rileva il volto, disegna solo l'interfaccia con l'ultima posizione nota del naso
                if show_window:
                    controller.draw_interface(frame, controller.last_nose_pos)
            
            # Mostra la finestra solo se show_window è True
            if show_window:
                ...
                # Se non mostriamo la finestra, mettiamo un piccolo ritardo per non saturare la CPU
                time.sleep(0.01) # Piccolo ritardo per ridurre l'utilizzo della CPU in background
        ...
        cap.release()
        cv2.destroyAllWindows() # Chiude tutte le finestre OpenCV, anche se non mostrate esplicitamente
        print("Pulizia completata. Il programma è terminato.")
