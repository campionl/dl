...
class HeadMouseController:
    ...
    def reset_mouse_position(self):
        """Riporta il cursore al centro dello schermo in modo thread-safe"""
        center_x, center_y = self.screen_w // 2, self.screen_h // 2
        with self.mouse_cursor.mouse_lock:
            self.mouse_cursor.current_mouse_pos = np.array([center_x, center_y], dtype=float)
            self.mouse_cursor.position_history.clear()
        try:
            pyautogui.moveTo(center_x, center_y)
            print(f"Puntatore riportato al centro: ({center_x}, {center_y})")
        except Exception as e:
            print(f"Errore nel riportare il puntatore al centro: {e}")
