...
class HeadMouseController:
    def __init__(self):
        ...
        # Acceleration thresholds are no longer used for clicks, but can be kept for other purposes
        self.ACCELERATION_THRESHOLD_X = 200
        self.ACCELERATION_THRESHOLD_Y = 200
        self.ACCELERATION_THRESHOLD_MAGNITUDE = 300

        # --- Variabili per Rilevamento Gesti Oculari/Bocca ---
        ...
        self.STILL_TIME_THRESHOLD = 1.0 # Seconds to stay still for left click
        self.last_movement_time = time.time()
        self.is_still = False
        self.STILL_MOVEMENT_THRESHOLD = 5 # Pixels for movement to count as "not still"
        # --- Fine Variabili per Rilevamento Gesti Oculari/Bocca ---


        self.last_left_click_time = 0.0
        self.last_right_click_time = 0.0
        self.last_double_click_time = 0.0
        self.CLICK_COOLDOWN = 0.5 # Cooldown to prevent multiple rapid clicks
    ...
    def detect_gestures(self, nose_pos, left_ear, right_ear, mouth_ratio):
        # Update nose position history for stillness detection
        ...
        # Check for stillness for left click
        if len(self.nose_pos_history) > 1:
            ...
            if movement > self.STILL_MOVEMENT_THRESHOLD:
                ...
                if (current_time - self.last_movement_time) >= self.STILL_TIME_THRESHOLD:
                    ...
                    self.is_still = False # Not still enough yet

        # Update eye and mouth history for winks and open mouth
    ...
    def draw_interface(self, frame, nose_pos, left_ear, right_ear, mouth_ratio, nose_acceleration):
        ...
        cv2.putText(frame, f"Stato: {still_status}", (20, 170), cv2.FONT_HERSHEY_SIMPLEX, 0.6, still_color, 1)

        cv2.putText(frame, "Mantieni FERMO: Click SX", (20, h-100),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
...
def main():
    print("="*60)
    print("HEAD MOUSE CONTROLLER ULTIMATE (STILLNESS/EYE/MOUTH GESTURES)")
    print("="*60)
    print("Sistema avanzato di controllo del mouse tramite movimenti del capo")
    print("Ottimizzato per fluidità e precisione con click basati su fermezza, occhi e bocca")
    print("="*60)
    ...
    try:
        # Acceleration is still calculated but primarily for internal filtering, not direct click gestures.
        # It's passed to draw_interface for consistency, but its values aren't central to clicks anymore.
        ...
        while True:
            ...
            if results.multi_face_landmarks:
                ...
                # Gesti basati su fermezza, occhi e bocca
                ...
                current_nose_acceleration = controller._calculate_nose_acceleration() # Still calculated for general use/debug
                ...
                controller.is_still = False # Reset stillness if face isn't detected


            cv2.imshow('Head Mouse Ultimate (Stillness/Eye/Mouth Gestures)', frame)
