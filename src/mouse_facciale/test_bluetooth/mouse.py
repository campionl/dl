...
class BluetoothHID:
    ...
    def setup_bluetooth_hid(self):
        ...
        try:
            ...
            self.ui = UInput(cap, name='RaspberryPi Head Mouse', version=0x3)
            print("Bluetooth HID device initialized")
        except Exception as e:
            print(f"Error initializing Bluetooth HID: {e}")
            sys.exit(1)
    ...
    def scroll(self, amount):
        """Send scroll event"""
        try:
            self.ui.write(ecodes.EV_REL, ecodes.REL_WHEEL, int(amount))
            self.ui.syn()
        except Exception as e:
...
class MouseCursorAction(BaseAction):
    ...
    def __init__(self, hid_device, screen_w, screen_h, sensitivity=4.0):
        self.hid = hid_device
    ...
    def update_position(self, direction, acceleration_factor, effective_distance):
        ...
        with self.mouse_lock:
            self.hid.move_mouse(smoothed_movement[0], smoothed_movement[1])
...
class ScrollAction(BaseAction):
    ...
    def __init__(self, hid_device, scroll_cooldown=0.03, sensitivity=2.0):
        self.hid = hid_device
...
class LeftClickAction(BaseAction):
    ...
    def __init__(self, hid_device, click_cooldown=0.5):
        self.hid = hid_device
...
class HeadMouseController:
    def __init__(self, show_window=True):
        ...
        # Screen dimensions
        self.screen_w, self.screen_h = 1920, 1080  # Default, will be updated
    ...
    def setup_event_action_mappings(self):
        ...
        for mapping in self.config['event_action_mappings']:
            ...
            if mapping['action_type'] == 'left_click':
                action = LeftClickAction(self.hid)
            elif mapping['action_type'] == 'right_click':
                action = RightClickAction(self.hid)
...
def main():
    ...
    print("=== RASPBERRY PI HEAD MOUSE CONTROLLER ===")
