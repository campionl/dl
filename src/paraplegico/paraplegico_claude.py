...
class HeadMouseController:
    def __init__(self):
        ...
        # Imposta il buffer a 1 per ridurre la latenza
    ...
    def apply_dead_zone_and_acceleration(self, dx, dy, head_velocity):
        ...
        # Calcola la distanza dal centro - NORMALIZZATA rispetto alle dimensioni dell'immagine
        # Questo deve corrispondere esattamente alla visualizzazione del cerchio
        distance = math.sqrt(dx*dx + dy*dy)
        
        # Zona morta - ora il confronto è coerente con la visualizzazione
    ...
    def draw_interface(self, image, nose_x, nose_y):
        ...
        # Disegna il centro di riferimento (punto verde)
        if self.center_nose_x and self.center_nose_y:
            ...
            cv2.circle(image, (center_pixel_x, center_pixel_y), 12, (0, 255, 0), 3)
            cv2.circle(image, (center_pixel_x, center_pixel_y), 6, (0, 255, 0), -1)

            # ZONA MORTA - CERCHIO GIALLO CHE RACCHIUDE ESATTAMENTE L'AREA DELLA ZONA MORTA
            # Il raggio deve corrispondere esattamente alla logica di self.dead_zone
            # self.dead_zone = 0.030 è una distanza normalizzata
            # Quindi il raggio in pixel deve essere: dead_zone * dimensione_riferimento
            dead_zone_radius_pixels = int(self.dead_zone * min(w, h))
            
            # Cerchio pieno semi-trasparente GIALLO per mostrare l'area morta
            overlay = image.copy()
            cv2.circle(overlay, (center_pixel_x, center_pixel_y), dead_zone_radius_pixels, (0, 255, 255), -1)
            cv2.addWeighted(overlay, 0.3, image, 0.7, 0, image)
            
            # Bordo del cerchio SPESSO per maggiore visibilità
            cv2.circle(image, (center_pixel_x, center_pixel_y), dead_zone_radius_pixels, (0, 255, 255), 4)
            
            # Aggiungi testo per chiarire
            cv2.putText(image, "ZONA MORTA", 
                       (center_pixel_x - 60, center_pixel_y - dead_zone_radius_pixels - 15), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)

        # Disegna la posizione corrente del naso - PIÙ GRANDE per visibilità
        nose_pixel_x = int(nose_x * w)
        nose_pixel_y = int(nose_y * h)
        if self.is_in_dead_zone:
            nose_color = (255, 255, 0)  # Ciano quando nella zona morta
            status_text = "FERMO"
        else:
            nose_color = (0, 0, 255)    # Rosso quando attivo
            status_text = "ATTIVO"
            
        cv2.circle(image, (nose_pixel_x, nose_pixel_y), 10, nose_color, 3)
        cv2.circle(image, (nose_pixel_x, nose_pixel_y), 4, nose_color, -1)
        
        # Testo accanto al naso
        cv2.putText(image, status_text, 
                   (nose_pixel_x + 15, nose_pixel_y - 10), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, nose_color, 2)
        
        # Informazioni di stato - TESTO PIÙ GRANDE
        ...
        # Stato zona morta con colori più evidenti
        if self.is_in_dead_zone:
            status_text = "ZONA MORTA - MOUSE FERMO"
            status_color = (0, 255, 255)  # Giallo
        else:
            status_text = "MOVIMENTO ATTIVO"
            status_color = (0, 255, 0)    # Verde
            
        cv2.putText(image, status_text, 
                   (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 1.0, status_color, 2)
        ...
        if self.calibration_count < self.calibration_frames:
            cv2.putText(image, f"Calibrazione: {self.calibration_count}/{self.calibration_frames}", 
                       (10, 160), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 255), 2)
            cv2.putText(image, "Mantieni la testa al centro!", 
                       (10, 200), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 255), 2)
        else:
            cv2.putText(image, "Calibrato - Controllo attivo", 
                       (10, 160), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)
            
        # Legenda colori - PIÙ GRANDE
        cv2.putText(image, "LEGENDA:", 
                   (10, h - 120), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        cv2.putText(image, "Verde = Centro | Giallo = Zona Morta", 
                   (10, h - 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        cv2.putText(image, "Ciano = Naso Fermo | Rosso = Naso Attivo", 
    ...
    def run(self):
        ...
        print("\nLegenda colori:")
        print("- Verde: Punto centrale di calibrazione")
        print("- Giallo: Zona morta (il mouse non si muove)")
        print("- Rosso: Posizione naso quando è attivo il movimento")
        print("- Ciano: Posizione naso quando è nella zona morta")
        
        # Imposta le proprietà della finestra per Arch Linux
        cv2.namedWindow('Controllo Mouse con Testa', cv2.WINDOW_NORMAL)
        cv2.resizeWindow('Controllo Mouse con Testa', 800, 600)
        ...
        try:
            while self.running:
                ...
                if results.multi_face_landmarks:
                    ...
                    if self.crop_bounds is not None:
                        ...
                        if face_crop.size > 0 and face_crop.shape[0] > 0 and face_crop.shape[1] > 0:
                            ...
                            # Ridimensiona il crop per riempire lo schermo (mantenendo proporzioni)
                            screen_height, screen_width = 600, 800  # Dimensioni finestra
                            face_crop_fullscreen = cv2.resize(face_crop_display, (screen_width, screen_height))
                            
                            # Disegna interfaccia sul crop a schermo intero
                            display_frame = self.draw_interface(face_crop_fullscreen, relative_nose_x, relative_nose_y)
                        else:
                            display_frame = self.draw_interface(frame, nose_x, nose_y)
                    else:
                        display_frame = self.draw_interface(frame, nose_x, nose_y)
                ...
                # Gestione input tastiera con timeout molto breve per evitare blocchi
