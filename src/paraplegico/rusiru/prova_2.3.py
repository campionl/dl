...
class SimpleNoseMouseController:
    def __init__(self):
        ...
        # Sistema di rilevamento ammiccamento per click - MODIFICATO per essere molto meno sensibile
        self.eye_closed_threshold = 0.25
        self.last_click_time = 0
        self.click_cooldown = 0  # Tempo minimo tra i click aumentato
        self.eye_aspect_ratios = []  # Buffer per stabilizzare il rilevamento
        self.ear_buffer_size = 15  # Buffer aumentato
        
        # Nuovo sistema per rilevamento più robusto
        self.blink_counter = 0
        self.blink_threshold = 5  # Numero di frame consecutivi sotto soglia aumentato
        self.eye_open_ear = None  # EAR di riferimento quando l'occhio è aperto
        self.eye_open_counter = 0  # Contatore apertura stabile
        self.open_threshold = 10  # Frame necessari per considerare l'occhio stabilmente aperto
    ...
    def detect_left_eye_blink(self, landmarks):
        """
        Rileva se l'occhio sinistro è chiuso e genera un click - VERSIONE MOLTO MENO SENSIBILE.
        """
        try:
            ...
            # Imposta EAR di riferimento solo se l'occhio è stabilmente aperto
            if self.eye_open_ear is None or (avg_ear > self.eye_open_ear * 0.9 and self.eye_open_counter >= self.open_threshold):
                self.eye_open_ear = avg_ear
                self.eye_open_counter = 0
            
            # Calcola soglia dinamica basata sull'EAR di riferimento (molto più restrittiva)
            dynamic_threshold = self.eye_open_ear * 0.55  # 55% dell'EAR normale
            
            current_time = time.time()
            
            # Logica di rilevamento blink migliorata
            if avg_ear < dynamic_threshold:
                self.blink_counter += 1
                self.eye_open_counter = 0  # Reset apertura stabile
            else:
                if self.blink_counter > 0:
                    self.blink_counter -= 1  # Decrescita graduale
                self.eye_open_counter += 1
            
            # Esegui click solo se:
            # 1. Blink confermato (sufficienti frame consecutivi)
            # 2. Cooldown passato
            # 3. Occhio era precedentemente aperto stabilmente
            if (self.blink_counter >= self.blink_threshold and 
                current_time - self.last_click_time > self.click_cooldown and
                self.eye_open_counter >= self.open_threshold // 2):
    ...
    def draw_interface(self, frame, nose_pos, landmarks=None, last_click_detected=False):
        ...
        if not self.center_calculated:
            ...
            cv2.putText(frame, "MOUSE ATTIVO + CLICK OCCHIO (RIDOTTA SENSIBILITÀ)", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, status_color, 2)
            ...
            if len(self.eye_aspect_ratios) > 0:
                avg_ear = np.mean(self.eye_aspect_ratios)
                dynamic_threshold = self.eye_open_ear * 0.55 if self.eye_open_ear else 0.25
                ...
                cv2.putText(frame, f"Blink Counter: {self.blink_counter}/{self.blink_threshold}", (20, 150),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                cv2.putText(frame, f"Open Counter: {self.eye_open_counter}/{self.open_threshold}", (20, 170),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
...
def main():
    print("="*60)
    print("CONTROLLORE MOUSE CON NASO + CLICK CON OCCHIO (SENSIBILITÀ RIDOTTA)")
    print("="*60)
    print("Avvio automatico senza calibrazione manuale.")
    print("Mantieni la testa ferma al centro all'inizio per alcuni secondi.")
    print("CLICK: Chiudi l'occhio SINISTRO PER ALMENO 0.5 SECONDI per fare click")
    print("MODIFICHE APPLICATE:")
    print("- Soglia blink al 55% dell'EAR normale (molto restrittiva)")
    print("- 8 frame consecutivi sotto soglia per confermare blink")
    print("- 1.2 secondi di cooldown tra i click")
    print("- Controllo apertura stabile dell'occhio prima del click")
    print("="*60)
    ...
    try:
        ...
        while True:
            ...
            cv2.imshow('Head Mouse + Eye Click (SENSIBILITÀ RIDOTTA)', frame)
