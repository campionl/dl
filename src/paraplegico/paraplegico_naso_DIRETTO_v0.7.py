...
class HeadMouseController:
    def __init__(self):
        ...
        self.MARGIN_FACTOR = 0.08

        self.nose_pos_history = deque(maxlen=90) # Increased history for stillness
        self.STILLNESS_HISTORY_LENGTH = 60 # Increased duration for stillness (1 second at 60 FPS)
        self.STILL_MOVEMENT_THRESHOLD_PIXELS = 2.5 # Slightly relaxed threshold for stillness

        self.calibrated_blink_threshold = 0.25
        self.calibrated_mouth_threshold = 0.40
        self.eye_history = deque(maxlen=15)
        self.mouth_history = deque(maxlen=15)
        self.EYE_MOUTH_HISTORY_LENGTH = 8

        self.STILL_TIME_THRESHOLD = 1.2
    ...
    def calibrate_movement_range(self, nose_pos, left_ear, right_ear, mouth_ratio):
        ...
        if self.calibration_stage == 8:
            ...
            if closed_eyes_data.size > 0:
                ...
                self.calibrated_blink_threshold = avg_closed_ear + 0.02
            ...
            if open_mouth_data.size > 0:
                ...
                self.calibrated_mouth_threshold = avg_open_mar * 0.8
            ...
            self.calibrated_blink_threshold = np.clip(self.calibrated_blink_threshold, 0.08, 0.30)
            self.calibrated_mouth_threshold = np.clip(self.calibrated_mouth_threshold, 0.30, 0.6)
