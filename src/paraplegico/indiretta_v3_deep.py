...
class HeadMouseController:
    ...
    def draw_interface(self, image, nose_x, nose_y):
        """Disegna l'interfaccia utente con indicatori"""
        h, w = image.shape[:2]
        ...
        if self.center_nose_x and self.center_nose_y:
            ...
            cv2.circle(image, (center_pixel_x, center_pixel_y), 8, (0, 255, 0), 2)  # Cerchio esterno verde
            cv2.circle(image, (center_pixel_x, center_pixel_y), 3, (0, 255, 0), -1)  # Pieno verde
        
        # Disegna la posizione attuale del naso
        nose_pixel_x = int(nose_x * w)
        nose_pixel_y = int(nose_y * h)
        
        # Colore del naso: giallo in zona morta, rosso altrimenti
        nose_color = (0, 255, 255) if self.is_in_dead_zone else (0, 0, 255)
        cv2.circle(image, (nose_pixel_x, nose_pixel_y), 6, nose_color, 2)
        
        # Disegna la zona morta (cerchio rosso)
        if self.center_nose_x and self.center_nose_y:
            # Calcola la distanza in pixel equivalente alla distanza euclidea normalizzata della dead zone
            dead_zone_radius = int(self.dead_zone * math.sqrt(w**2 + h**2))
            cv2.circle(image, (center_pixel_x, center_pixel_y), dead_zone_radius, (0, 0, 255), 2)
        
        # Informazioni di stato
        cv2.putText(image, f"Mouse: ({int(self.mouse_x)}, {int(self.mouse_y)})", 
                   (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        
        # Mostra la velocità della testa
