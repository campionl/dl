...
class HeadMouseController:
    def __init__(self):
        ...
        self.MARGIN_FACTOR = 0.08 # Decreased for higher sensitivity

        self.nose_pos_history = deque(maxlen=60)
        self.STILLNESS_HISTORY_LENGTH = 40 # Increased for more robust stillness detection
        self.STILL_MOVEMENT_THRESHOLD_PIXELS = 2.0 # Decreased for stricter stillness

        self.calibrated_blink_threshold = 0.25
        self.calibrated_mouth_threshold = 0.40
        self.eye_history = deque(maxlen=15) # Increased for more reliable blink detection
        self.mouth_history = deque(maxlen=15) # Increased for more reliable mouth open detection
        self.EYE_MOUTH_HISTORY_LENGTH = 8 # Increased to require more sustained gestures

        self.STILL_TIME_THRESHOLD = 1.2 # Increased for more deliberate left click
        ...
        self.CLICK_COOLDOWN = 0.8 # Slightly increased to prevent rapid false clicks

        self.FILTER_STRENGTH = 0.15 # Slightly decreased for a touch more responsiveness
    ...
    def calibrate_movement_range(self, nose_pos, left_ear, right_ear, mouth_ratio):
        ...
        if self.calibration_stage == 8:
            ...
            if closed_eyes_data.size > 0:
                ...
                self.calibrated_blink_threshold = avg_closed_ear + 0.02 # Slightly above closed average for sensitivity
            ...
            if open_mouth_data.size > 0:
                ...
                self.calibrated_mouth_threshold = avg_open_mar * 0.8 # More strict, closer to open mouth for trigger
            ...
            self.calibrated_blink_threshold = np.clip(self.calibrated_blink_threshold, 0.08, 0.30) # Tighter clip range
            self.calibrated_mouth_threshold = np.clip(self.calibrated_mouth_threshold, 0.30, 0.6) # Tighter clip range
    ...
    def draw_interface(self, frame, nose_pos, left_ear, right_ear, mouth_ratio):
        ...
        if self.calibrated:
            ...
            cv2.putText(frame, "Mantieni FERMO (>=1.2s): Click SX", (20, h-100),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
